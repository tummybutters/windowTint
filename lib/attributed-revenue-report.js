...
// array with one entry per currency actually present, not one scalar.
const groupAnomalyRows = (anomalyRows) => {
  const grouped = { currencyMismatch: [], orderNotInReport: [], unresolvedRefunds: [] };
  ...
  for (const key of Object.values(ANOMALY_TYPE_KEYS)) {
    grouped[key].sort((a, b) => a.currency.localeCompare(b.currency));
  }
  return grouped;
};
