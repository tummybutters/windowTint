...
-- output with a table lookup so a touch created and linked in the same call is found either way.
touch_lookup AS (
  SELECT touch_id, gclid, gbraid, wbraid FROM touch_insert
  UNION ALL
  SELECT touch_id, gclid, gbraid, wbraid FROM attribution_touches WHERE touch_id = NULLIF($63, '')
),
lead_intent_upsert AS (
  ...
  SELECT $61, $62, $1, NULLIF($63, ''), $30, $64, NOW()
  WHERE $61 <> ''
