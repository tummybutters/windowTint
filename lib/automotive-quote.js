...
function normalizeQuote(input){
 ...
 if(!['Car','SUV / crossover','Truck','Supercar','Other'].includes(out.type)||!['Front two windows','Sides + rear','Windshield / other','Help me choose'].includes(out.coverage))throw Error('invalid_choice');
 ...
 out.consent_version='callback-2026-09-20';out.payload_hash=crypto.createHash('sha256').update(JSON.stringify({type:out.type,coverage:out.coverage,vehicle:out.vehicle,address:out.address,name:out.name,phone:out.phone,contact:out.contact})).digest('hex');return out;
}
