...
WITH inserted AS (
  ...
  RETURNING lead_id, name, phone, property_city, additional_notes, answers
)
SELECT lead_id, name, phone, property_city, additional_notes, answers, TRUE AS inserted FROM inserted
UNION ALL
SELECT lead_id, name, phone, property_city, additional_notes, answers, FALSE AS inserted
...
const ANSWER_FIELDS = ['property', 'goal', 'scope', 'timing'];

const sameCustomerMessage = (row, record) => (
  String(row.name || '') === record.name
  && String(row.phone || '') === record.phone
  && String(row.property_city || '') === record.property_city
  && String(row.additional_notes || '') === record.additional_notes
  && ANSWER_FIELDS.every((field) => String((row.answers || {})[field] || '') === record.answers[field])
);
...
const createCommercialLeadStore = ({ query }) => {
  ...
  return {
    ...
    async persist(record) {
      ...
      if (!sameCustomerMessage(row, record)) {
        const error = new Error('Submission ID already belongs to different project information');
        error.code = 'commercial_lead_conflict';
        throw error;
      }
