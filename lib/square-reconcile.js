...
const toSnakeCaseKey = (key) => key.replace(/[A-Z]/g, (letter, offset) => (
...
const canonicalize = (value) => {
  ...
  if (value && typeof value === 'object') {
    ...
    const bySnakeKey = {};
    // Explicit snake_case (or already-lowercase) keys are assigned first so they
    // win over a camelCase alias for the same field (SDK objects can carry both).
    for (const key of keys) {
      if (toSnakeCaseKey(key) === key) bySnakeKey[key] = value[key];
    }
    for (const key of keys) {
      const snakeKey = toSnakeCaseKey(key);
      if (snakeKey !== key && !Object.prototype.hasOwnProperty.call(bySnakeKey, snakeKey)) {
        bySnakeKey[snakeKey] = value[key];
      }
    }
    return Object.keys(bySnakeKey).sort().reduce((result, key) => {
      result[key] = canonicalize(bySnakeKey[key]);
      return result;
    }, {});
  }
