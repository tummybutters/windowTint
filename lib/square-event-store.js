...
entity_upsert AS (
  INSERT INTO attribution_payments (
    payment_id, provider, provider_payment_id, customer_identity_hash, status,
    amount_minor, currency, provider_created_at, metadata, last_provider_event_at, updated_at
  )
  SELECT $5, 'square', $6, NULLIF($7, ''), NULLIF($8, ''),
    $9, $10, NULLIF($11, '')::timestamptz, $12::jsonb, $13::timestamptz, NOW()
  FROM receipt_insert
  ...
  if (record.entity_type === 'payment') {
    return {
      ...
      params: [...baseParams(record), entity.payment_id, entity.provider_payment_id,
        entity.customer_identity_hash, entity.status, entity.amount_minor, entity.currency,
        entity.provider_created_at, JSON.stringify(entity.metadata || {}),
        record.provider_created_at]
    };
  }
