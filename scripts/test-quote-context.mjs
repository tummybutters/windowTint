...
for(const [path,vehicle,type] of [['/tesla-model-3-window-tinting','Tesla Model 3','Car'],['/tesla-model-y-window-tinting','Tesla Model Y','SUV / crossover'],['/tesla-cybertruck-window-tint','Tesla Cybertruck','Truck']]){
 const c=contextFor(path);assert.deepEqual(c.steps,[3,1,4,2]);assert.equal(c.answers.vehicle,vehicle);assert.equal(c.answers.type,type);
}
...
assert.deepEqual(contextFor('/car-window-tinting-near-me').steps,[0,3,1,4,2]);
const removal=contextFor('/tint-removal');assert.deepEqual(removal.steps,[0,3,4,2]);assert.equal(removal.answers.coverage,'Tint removal');
