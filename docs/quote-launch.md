...
Server-only variables: TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER, QUOTE_ALERT_TO, QUOTE_RETRY_SECRET, AUTOMOTIVE_QUOTES_ENABLED, DATABASE_URL, ATTRIBUTION_HMAC_SECRET, TINT_WIZ_ZAPIER_HOOK_URL. Credentials are not in source or client JavaScript.
...
The September 22 Tint Wiz extension adds a second outbox on saved automotive quotes. New records start with `crm_state=pending`; older records remain `ignored` so launching the Zap does not flood Tint Wiz with historical or duplicate contacts. A private server-only Zapier hook sends customer name, callback number, service address, vehicle, desired windows, priority, timing, and quote ID to Tint Wiz. Twilio remains the single alert to Elle. Zapier acceptance is recorded separately from a confirmed Tint Wiz contact; the live Zap's test created a labeled contact successfully. Timeout or server uncertainty stays `unknown` for review, while explicit rate limits can retry. The existing worker reports CRM failures alongside Twilio failures.
